  ...
  return (
    <div className="grid md:grid-cols-[280px_1fr] gap-4">
        ...
        {conversations.map((c) => (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <span className="font-medium text-slate-800 line-clamp-2">{c.title}</span>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  void remove(c.id);
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 min-h-64">
        {!selected ? (
          <p className="text-slate-400 text-sm">Select a session to view its transcript.</p>
        ) : (
          <div className="space-y-3">
            {messages.map((m) => (
              <div key={m.id} className="border-b border-slate-100 pb-3 last:border-0">
                ...
                <p className="text-slate-900">🇺🇦 {m.translated_text}</p>
